...
public:
  ...
  // 所以这么做完全不会有问题，只是为了满足标准容器的 rebind / allocator_traits
  // 要求
  ...
  template <typename U, typename... Args> U *construct(U *p, Args &&...args);
...
// construct 是 allocator 的成员函数，所以内层也要 template<typename U,
// typename... Args>
template <typename T>
...
template <typename T> template <typename U> void allocator<T>::destroy(U *p) {
  if (p == nullptr)
    return;
  p->~U();
}
...
// 对于无状态allocator来说，标准写法是， "所有 allocator<T> 和 allocator<U>
// 实例，哪怕 T ≠ U，也认为是可互换的"
